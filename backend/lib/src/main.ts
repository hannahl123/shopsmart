...
function loadData(center: string, radius: number, requirements: Map<string,number>) {
    ...
    for (let i = 0; i < companies.length; i++) { // for each company
        let company = companies[i];
        companiesByName[company.name] = company;
        for (let j = 0; j < company.stores.length; j++) { // for each store
            let store = company.stores[j];
            store.productsByName = new Map(); // map store by name for faster access
            for (let k = 0; k < store.products.length; k++) { // for each product
                store.productsByName.set(store.products[k].name, store.products[k]);
            }
            if (locations.distance(center, store.location) <= radius) { // store within radius
                let flag = false; // contributes requirement?
                for (let [key, val] of Array.from(requirements.entries())) { // contributes requirement
                    if (store.productsByName.has(key)) {flag = true; break;}
                }
                if (flag) { // contributes requirement
                    stores.push(store);
